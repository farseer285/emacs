...
;; `char-displayable-p' was renamed from `latin1-char-displayable-p' in
;; Emacs 22
(if (fboundp 'char-displayable-p)
    (defalias 'visws-char-displayable-p 'char-displayable-p)
  (require 'latin1-disp)
  (defalias 'visws-char-displayable-p 'latin1-char-displayable-p))
...
(defun visws-legal-display-vector-p (vec)
  ...
  (let ((i 0) (len (length vec)))
    (while (and (< i len) (visws-char-displayable-p (aref vec i)))
      (setq i (1+ i)))
    (= i len)))
